...
export const VENDORS = [
  ...
  },
  {
    ...
    parse: (x) => {
      ...
      if (!d) return { error: 'Unrecognized response format' }
  ...
  },
  {
    ...
    parse: (x) => {
      ...
      if (resp && resp.status_code && resp.status_code !== 0) {
        return { error: resp.status_msg || 'MiniMax balance request failed' }
      }
      ...
      return { quota: quota.length > 0 ? quota : ['No quota data'], currency: 'CNY' }
    }
  },
  {
    id: 'stepfun',
    label: 'StepFun',
     labelZh: '阶跃星辰 StepFun',
    match: /stepfun/,
  ...
  },
  {
    ...
    quotaOnly: true,
    parse: (x) => {
      if (x && x.code !== undefined && x.code !== 200) return { error: x.msg || 'Zhipu quota request failed' }
  ...
  },
  {
    ...
    quotaOnly: false,
    parse: (x) => {
      if (x && x.error) return { error: typeof x.error === 'string' ? x.error : 'SiliconFlow balance request failed' }
