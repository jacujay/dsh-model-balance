        ...
        handler: async (request, response) => {
          ...
          sendJson(response, 200, {
            vendors: VENDORS.map((vendor) => ({
              id: vendor.id,
              label: vendor.label,
              labelZh: vendor.labelZh || vendor.label,
      ...
      webServer.register({
        ...
        handler: async (request, response) => {
          ...
          if (request.method === 'POST') {
            ...
            if (!provider) { sendJson(response, 400, { error: 'provider id is required' }); return }
            const url = String(body.url || '').trim()
            if (!url) { sendJson(response, 400, { error: 'endpoint URL is required' }); return }
            const path = String(body.path || '').trim()
            if (!path) { sendJson(response, 400, { error: 'response field path is required' }); return }
