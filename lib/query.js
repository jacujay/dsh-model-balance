...
function failure(provider, model, error) {
  return { status: 'error', provider: provider || '', model: model || '', error: error || '余额查询失败' }
}

export async function queryBalance(ctx) {
  const route = currentRoute(ctx)
  if (!route.provider) return failure('', route.model, '未配置默认模型')
  ...
  if (customEntry) {
    ...
    return {
      ...
      error: '该厂商没有公开的余额查询接口'
    }
  }

  if (!endpoint) return failure(route.provider, route.model, '未配置余额接口地址')
  if (!apiKeyEnv) return failure(route.provider, route.model, '未配置 API Key 环境变量')

  const credentials = ctx.get('credentials')
  if (credentials === undefined) return failure(route.provider, route.model, '凭据服务不可用')
  const hit = await credentials.resolve(apiKeyEnv)
  if (hit === undefined) return failure(route.provider, route.model, `未配置 ${apiKeyEnv}`)
  ...
  try {
    ...
    return failure(route.provider, route.model, message.slice(0, 120) || '余额接口请求失败')
  }
